...
export const smaCrossoverSchema: z.ZodType<Condition> = z.object({
  type: z.literal(TYPE),
  params: z.object({
    fast_period: z.coerce.number().int().min(2).max(1000),
    slow_period: z.coerce.number().int().min(2).max(1000),
    direction: z.enum(['fast_crossing_above_slow', 'fast_crossing_below_slow']),
  }),
})

registerConditionSchema(TYPE, smaCrossoverSchema)
